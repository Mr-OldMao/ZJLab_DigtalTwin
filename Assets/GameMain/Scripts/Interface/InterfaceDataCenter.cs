            ...
            string jsonStr = @"
{
    ...
    public void CacheGetThingGraph(string id, Action callbackSuc = null, Action callbakcFail = null)
    {
        if (!MainData.CanReadFile)
        {
            ...
            if (MainData.getThingGraph == null)
            {
            ...
            {
                Debugger.Log("MainData.getThingGraph 不为空 之间使用缓存数据，物体与房间的邻接关系");
                //删除“吊灯”数据
                foreach (GetThingGraph_data_items roomData in MainData.getThingGraph.data.items)
                {
                    //Debugger.Log(roomData.name + "_" + roomData.id);
                    for (int i = 0; i < roomData.relatedThing.Count;)
                    {
                        GetThingGraph_data_items_relatedThing item = roomData.relatedThing[i];
                        //Debugger.Log(roomData.name + "_" + roomData.id + ",item:" + item.target.name + "_" + item.target.id);
                        if (item.target.name == "Toplamp")
                        {
                            roomData.relatedThing.Remove(item);
                        }
                        else
                        {
                            i++;
