...
public class GameLogic : SingletonByMono<GameLogic>
{
    ...
    /// 房间尺寸随机数 长min，max
    /// </summary>
    private int[] m_RoomSizeRandomLength = new int[2] { 6, 10 };
    /// <summary>
    /// 房间尺寸随机数 长min，max
    /// </summary>
    private int[] m_RoomSizeRandomWidth = new int[2] { 6, 10 };
...
#if UNITY_EDITOR 
        paramStr = "Simulator:1703552696880|1";// "WinPC_" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + "|" + "1";  //"Simulator:1700126538734|1"
    ...
    private void RegisterMsgEvent()
    {
        MsgEvent.RegisterMsgEvent(MsgEventName.GenerateSceneComplete, () =>
        {
            ...
            //CanUpadeteSceneEntityInfo = string.IsNullOrEmpty(MainData.ConfigData.CoreConfig.LocalReadFileName) && !MainData.UseTestData;
